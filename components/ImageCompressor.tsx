...
export default function ImageCompressor() {
  ...
  return (
    <div ref={compressorRef} className="h-full flex flex-col bg-[#0e0e0e] text-white overflow-hidden">
      {/* ---- Main Content: Upload Zone + Right Panel ---- */}
      <div className="flex-1 flex flex-col lg:flex-row gap-3 overflow-hidden p-4">
        {/* ---- Left: Upload Zone or Image Display ---- */}
        {images.length === 0 ? (
          <div
          ...
          <div className="bg-[#151515] rounded-xl p-3 space-y-2.5">
            ...
            {activeAlgo === 'pngquant' && (
              <div className="space-y-2">
                ...
                <button onClick={() => rerunAlgo('pngquant')} disabled={processing.pngquant || !selectedImage}
                  className="w-full py-1.5 text-[10px] font-medium rounded-lg bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  {processing.pngquant ? '压缩中...' : selectedImage ? '重新压缩' : '请先上传图片'}
                </button>
              </div>
            )}
            ...
            {activeAlgo === 'oxipng' && (
              <div className="space-y-2">
                ...
                <input type="range" min={1} max={6} step={1} value={oxiLevel}
                  onChange={e => setOxiLevel(Number(e.target.value))}
                  className="w-full h-1 bg-[#333] rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-green-500" />
                <button onClick={() => rerunAlgo('oxipng')} disabled={processing.oxipng || !selectedImage}
                  className="w-full py-1.5 text-[10px] font-medium rounded-lg bg-green-500/10 text-green-500 hover:bg-green-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  {processing.oxipng ? '优化中...' : selectedImage ? '重新优化' : '请先上传图片'}
                </button>
              </div>
            )}
            ...
            {activeAlgo === 'zopfli' && (
              <div className="space-y-2">
                ...
                {zopfliEngine === 'available' ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      disabled={processing.zopfli || !selectedImage}
                      className="w-full py-1.5 text-[10px] font-medium rounded-lg border border-dashed border-[#333] text-[#666] hover:text-blue-400 hover:border-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                      重试 Zopfli WASM
                    </button>
                  </>
                )}
                ...
                {processing.zopfli && zopfliProgress > 0 ? (
                  ...
                  <button onClick={() => rerunAlgo('zopfli')} disabled={processing.zopfli || !selectedImage}
                    className="w-full py-1.5 text-[10px] font-medium rounded-lg bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                    {selectedImage ? '重新压缩' : '请先上传图片'}
                  </button>
                ) : (
                  <div className="flex items-center justify-center gap-2 py-1">
                    <Loader2 className="w-3 h-3 text-blue-500 animate-spin" />
            ...
            {activeAlgo === 'webp' && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-[#555]">{webpLossless ? '像素级无损，压缩率通常优于 PNG' : '越低体积越小，越高画质越好'}</p>
                <button onClick={() => rerunAlgo('webp')} disabled={processing.webp || !selectedImage}
                  className="w-full py-1.5 text-[10px] font-medium rounded-lg bg-cyan-500/10 text-cyan-500 hover:bg-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  {processing.webp ? '编码中...' : selectedImage ? '重新编码' : '请先上传图片'}
                </button>
              </div>
            )}
            ...
            {activeAlgo === 'avif' && (
              <div className="space-y-2">
                ...
                <input type="range" min={1} max={100} value={avifQuality}
                  ...
                  className="w-full h-1 bg-[#333] rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-purple-500" />
                <p className="text-[10px] text-[#555]">80+ 肉眼几乎无损，体积远小于 PNG</p>
                <button onClick={() => rerunAlgo('avif')} disabled={processing.avif || !selectedImage}
                  className="w-full py-1.5 text-[10px] font-medium rounded-lg bg-purple-500/10 text-purple-500 hover:bg-purple-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  {processing.avif ? '编码中...' : selectedImage ? '重新编码' : '请先上传图片'}
                </button>
              </div>
            )}
          </div>
